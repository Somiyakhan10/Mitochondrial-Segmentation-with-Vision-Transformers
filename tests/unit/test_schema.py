...
def _make_result(analysis_id="A1", time_point="6 weeks", health_score=52.3) -> AnalysisResult:
    return AnalysisResult(
        ...
        image_info=ImageInfo(
            filename="m.tif", experiment="SCI 6 weeks", animal_id="M123", time_point=time_point
        ),
        ...
        compared_to_baseline=BaselineComparison(
            condition="SCI vs Naive", significance="p < 0.01", change_percent=-32.5
        ),
    )
...
def test_timeseries_sorts_time_points_numerically():
    ...
    series.add_measurement(
        "21 weeks", _make_result(analysis_id="A2", time_point="21 weeks", health_score=30.0)
    )
